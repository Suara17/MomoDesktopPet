...
一个生动、轻量且可爱的 Android 顶层悬浮桌宠客户端。支持丰富的序列帧微动作、手势交互拖拽、学习与专注模式、沉浸式音效反馈、多级震动提醒、应用沉迷监督提醒、音乐陪伴、迷你模式，以及智能贴边停靠探头。
...
4. **细腻多级触感与震动提醒（Vibrator / VibrationEffect）**：
   - 区分日常计时结束、手动结束、防沉迷初次提醒以及严重超时不同场景，定制不同节奏与力度的多段脉冲震动，即使静音状态下也不会错过重要提醒。
5. **应用沉迷监督与贴心提醒**：
   - 支持使用情况访问权限（UsageStats），针对短视频、社交等易沉迷应用进行时长检测，配合气泡、提示音与警示震动进行多维防沉迷监督。
6. **学习与休闲计时模式**：
...
9. **逐帧状态机与生活化事件**：
   - 包含眨眼、发呆、喝水、抱猫、专注计时以及超时入睡等丰富的生活状态机制。
